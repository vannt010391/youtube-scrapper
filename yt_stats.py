...
class YTstats():
    ...
    async def dump(self, channel_name):
        ...
        filename = channel_name + '.json'
        with open(filename, 'w', encoding="utf-8") as f:
            json.dump(fused_data, f, indent=4)
        return filename

    async def format_file(self, channel_name, file):
        outputFile = channel_name + '.csv'
...
### Get channel_id
async def getChannelIdFromCustomUrl(channel_name):
    url = f'https://www.youtube.com/c/{channel_name}'
    # url = f'channelUsername'
...
async def app(api_key, channel_name) -> None:
    channel_id = await getChannelIdFromCustomUrl(channel_name)        
    youtube_channel = YTstats(api_key, channel_id)
    await youtube_channel.extract_all()
    file = await youtube_channel.dump(channel_name)
    await youtube_channel.format_file(channel_name, file)
